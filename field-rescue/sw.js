...
const CACHE_NAME = "field-rescue-lkg-2026-08-06-e82c1316";
...
self.addEventListener("activate", event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(key => key.startsWith("field-rescue-lkg-") && key !== CACHE_NAME).map(key => caches.delete(key))))
