...
async function main() {
  ...
  assert(reportFiles.has("CHATGPT_START_HERE.md"), "the analysis ZIP has an unmistakable first file for ChatGPT");
  ...
  assert(reportFiles.get("inspection.json").length < Buffer.byteLength(JSON.stringify(reportManifest, null, 2) + "\n"), "report JSON is compacted without changing its records");
