...
const tradeoffLinks = ValueEngine.linksFromContext({ driver_ids: ["water"], effects: ["increase_value", "increase_cost"], magnitude: 4, confidence: "medium", reason: "The creek is an amenity that may also add constraints." });
...
const observations = [
  ...
  observation("homesite-1", "homesite", 30.4903, -87.0897, [{
    ...
    inspector_reason: "The inspector identified a plausible candidate homesite.", assessment_source: "inspector_selected"
  }]),
  observation("tree-1", "tree", 30.4905, -87.0895, [{
    ...
    inspector_reason: "A mature tree could strengthen homesite character if safely preserved.", assessment_source: "inspector_selected"
  }]),
...
assert(app.includes('const APP_VERSION = "3.14.0"'));
...
assert(worker.includes("property-value-engine.js?v=3.14.0") && worker.includes("v20"), "the value engine is available completely offline");
