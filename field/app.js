(function () {
  ...
  async function createAnalysisJpeg(file) {
    ...
    try {
      ...
      const blob = await new Promise((resolve, reject) => {
        canvas.toBlob(value => value ? resolve(value) : reject(new Error("The analysis JPEG could not be created.")), "image/jpeg", 0.75);
      });
      return { blob, width: decoded.width, height: decoded.height, analysisWidth: width, analysisHeight: height, maxDimension, jpegQuality: 0.75 };
    } finally {
      decoded.close();
    }
  }
  ...
  async function shareLastPackage() {
    ...
    try {
      await navigator.share({
        ...
        text: "Choose Save to Dropbox. The inspection will synchronize to the analysis computer automatically."
      });
      setStatus("The iPhone share sheet accepted the ZIP. Keep this inspection until you confirm the file is saved.", "success");
      return true;
    } catch (error) {
      if (error && error.name !== "AbortError") setStatus("The share sheet did not accept the ZIP. Your inspection is still safe. Tap SEND ZIP NOW and choose Save to Dropbox.", "warning");
      return false;
    }
  }
  ...
  async function presentPackage(name, blob, manifest) {
    ...
    packageLink.textContent = "SAVE A BACKUP TO FILES";
    packageLink.hidden = false;
    packageFilename.textContent = name;
    packageInstruction.textContent = reportPackage ? "NEXT: Tap SEND ZIP NOW. On the next screen, tap Save to Dropbox. That is all." : "NEXT: Tap SAVE A BACKUP TO FILES. Keep this preservation archive; it may be too large for ChatGPT.";
    ...
    sharePackageBtn.textContent = reportPackage ? "SEND ZIP NOW" : "SHARE OR SAVE ARCHIVE";
    sharePackageBtn.hidden = !canShareFile;
    updateNextStep();
    if (canShareFile) {
      setStatus(reportPackage ? "Inspection ZIP is ready. Tap SEND ZIP NOW, then tap Save to Dropbox. That is all." : "Full archive is ready. Save it in a permanent location.", "success");
    } else {
  ...
  async function recoverEveryPhoto(packageMode) {
    ...
    for (let index = 0; index < data.photos.length; index += 1) {
      ...
      const reportReady = analysisProfile && Number(analysisProfile.max_dimension_px) <= 1600 && Number(analysisProfile.jpeg_quality) === 0.75;
