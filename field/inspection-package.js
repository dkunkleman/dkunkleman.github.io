...
${questions.map(question => `## ${question.category}\n\n${question.question}\n\n- Audience: ${question.stakeholder}\n- Decision: ${question.decision_id || "Cross-cutting"}\n- Evidence present: ${question.evidence_present ? "Yes" : "No or not specifically recorded"}\n- Supporting observations: ${question.supporting_observation_ids.join(", ") || "None specifically linked"}\n- Output rule: Answer when supported. Otherwise state the uncertainty and the lowest-cost credible way to resolve it.\n`).join("\n")}`;
  ...
  async function createInspectionPackage(options) {
    ...
    const packageJson = value => JSON.stringify(value, null, includeOriginals ? 2 : 0) + "\n";
    zip.add("AI_README.md", aiReadme, { modifiedAt });
    zip.add("CHATGPT_START_HERE.md", aiReadme, { modifiedAt });
    zip.add("AI_ANALYSIS.json", packageJson(aiAnalysis), { modifiedAt });
    ...
    zip.add("SMALL_TRACT_WATER_MAP.json", packageJson(smallTractWaterMap), { modifiedAt });
    zip.add("FLOWING_WATER_CORRIDORS.json", JSON.stringify(flowingWaterCorridorModel, null, 2) + "\n", { modifiedAt });
    zip.add("SEGMENTED_ROUTE.json", packageJson(reviewedSynthesis.route), { modifiedAt });
    ...
    zip.add("CREEK_CORRIDOR_MAP.json", packageJson(reviewedSynthesis.creek_corridor_map), { modifiedAt });
    zip.add("creek-corridor-map.html", reviewedSynthesis.map_html.creek, { modifiedAt });
    zip.add("VEGETATION_CLEARING_MAP.json", packageJson(reviewedSynthesis.vegetation_clearing_map), { modifiedAt });
    zip.add("vegetation-clearing-map.html", reviewedSynthesis.map_html.vegetation, { modifiedAt });
    zip.add("HOMESITE_OPPORTUNITY_MAP.json", packageJson(reviewedSynthesis.homesite_opportunity_map), { modifiedAt });
    ...
    zip.add("EVIDENCE_RELATIONSHIPS.json", packageJson(evidenceRelationships), { modifiedAt });
    ...
    zip.add("repository-comparison.json", packageJson(comparisonRecord), { modifiedAt });
    zip.add("schema.json", JSON.stringify(schema, null, 2) + "\n", { modifiedAt });
    zip.add("inspection.json", packageJson(manifest), { modifiedAt });
    ...
    zip.add("photo_index.json", packageJson(photoIndex), { modifiedAt });
