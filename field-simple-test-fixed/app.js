(function () {
  ...
  function renderSmallTractWaterMap() {
    ...
    if (waterLayerEnabled("terrain")) addWaterSvg("image", { href: "../field-simple-test/assets/usgs-terrain.png", x: 0, y: 0, width: W, height: H, preserveAspectRatio: "none" });
    if (waterLayerEnabled("contours")) addWaterSvg("image", { href: "../field-simple-test/assets/usgs-contours-2ft.png", x: 0, y: 0, width: W, height: H, preserveAspectRatio: "none", opacity: .78 });
  ...
  async function loadParcels() {
    try {
      const response = await fetch("../field-simple-test/assets/parcels.json", { cache: "force-cache" });
  ...
  function markerFromPosition(type, note, photoId, time, positionOverride, details) {
    ...
    return {
      ...
      lat: Number.isFinite(Number(position.lat)) ? Number(position.lat) : null,
      lon: Number.isFinite(Number(position.lon)) ? Number(position.lon) : null,
      gps_accuracy_m: Number.isFinite(Number(position.accuracy_m)) ? Number(position.accuracy_m) : null,
      gps_position_at: position.time || null,
      compass_heading_deg: latestOrientation ? latestOrientation.compass_heading_deg : (position.heading_deg == null ? null : position.heading_deg),
      device_orientation: latestOrientation ? {
        alpha_deg: latestOrientation.alpha_deg,
        beta_deg: latestOrientation.beta_deg,
        gamma_deg: latestOrientation.gamma_deg,
  ...
  async function startVoiceRecording(options) {
    simpleSetStatus("VOICE RECORDING IS OFF. Use ADD NOTE and the microphone on the iPhone keyboard.", "warning");
    return;
    const settings = options || {};
  ...
  async function recoverMapContext() {
    ...
    const [terrainBlob, contourBlob, parcelsText] = await Promise.all([
      fetchBlob("../field-simple-test/assets/usgs-terrain.png"),
      fetchBlob("../field-simple-test/assets/usgs-contours-2ft.png"),
      fetchParcels()
    ]);
  ...
  const simpleFeatureNames = {
    ...
    open: "OPEN AREA", highlow: "HIGH / LOW", other: "OTHER", photo: "PHOTO", note: "NOTE",
  ...
  function latestSavedPosition() {
    if (lastPosition) return lastPosition;
    const point = data.points.length ? data.points[data.points.length - 1] : null;
    if (!point) return null;
    return {
      lat: Number(point.lat), lon: Number(point.lon), accuracy_m: point.accuracy_m,
      heading_deg: point.heading_deg, time: point.time || null
    };
  }

  function noteReturnScreen(settings) {
    const sectionId = settings && settings.section_id;
    if (sectionId && sectionMappingTools) {
      const section = sectionMappingTools.sectionById(data, sectionId);
      if (section) {
    ...
    if (settings && settings.return_screen && settings.return_screen !== "FIELD_BUTTONS") {
      setFrontageScreen(settings.return_screen);
      renderFrontageWorkflow();
      return;
    }
    if (settings && settings.session_id) {
      const session = simpleSessionById(settings.session_id);
      if (session) { simpleActiveSessionId = session.simple_session_id; data.active_simple_session_id = session.simple_session_id; renderSimpleCapture(); return; }
    }
    renderSimpleHome();
  }

  function renderNoteEditor(options) {
    const settings = Object.assign({ section_id: null, feature_id: null, observation_id: null, purpose: "GENERAL_FIELD_NOTE" }, options || {});
    const savedDraft = (() => {
      try { return JSON.parse(localStorage.getItem(noteDraftKey) || "null"); } catch (error) { return null; }
    })();
    const draftMatches = savedDraft && String(savedDraft.section_id || "") === String(settings.section_id || "") && String(savedDraft.feature_id || "") === String(settings.feature_id || "");
    const existingText = draftMatches ? savedDraft.text || "" : settings.existing || "";
    const existingPurposes = draftMatches && Array.isArray(savedDraft.purpose_choices) ? savedDraft.purpose_choices : [];
    const sectionPurposes = ["POTENTIAL HOUSE OPENING", "POSSIBLE ADDITIONAL HOMESITE", "POSSIBLE PASTURE OR OPEN AREA", "CREEK OR WATER FEATURE", "ACCESS OR ENTRANCE", "VEGETATION / CLEARING AREA", "OTHER"];
    const content = document.getElementById("simpleContent");
    content.innerHTML = `<section class="simple-capture note-editor"><h2>${settings.title || (settings.section_id ? "WHY I MAPPED THIS SECTION" : "ADD NOTE")}</h2><p class="frontage-instruction">Type here, or tap the microphone on the iPhone keyboard to dictate. The app does not record audio.</p><div class="simple-calculation">GPS: ${latestSavedPosition() ? `available, accuracy about ${Math.round(latestSavedPosition().accuracy_m || 0)} m` : "not currently available - the note will still save"}<br>Section: ${settings.section_id || "none"}</div>${settings.section_id ? `<fieldset class="section-purpose-choices"><legend>Optional purpose</legend>${sectionPurposes.map(choice => `<label><input type="checkbox" value="${choice}" ${existingPurposes.includes(choice) ? "checked" : ""}> ${choice}</label>`).join("")}</fieldset>` : ""}<label>YOUR NOTE<textarea id="ordinaryNoteText" rows="8" placeholder="Type or dictate why this matters"></textarea></label><p id="ordinaryNoteState" class="note-not-saved">NOT YET SAVED</p><div class="simple-capture-actions"><button id="saveOrdinaryNote" class="frontage-end" type="button">${settings.save_label || "SAVE NOTE"}</button>${settings.allow_without_note ? `<button id="saveWithoutSectionNote" type="button">SAVE SECTION WITHOUT NOTE</button>` : ""}<button id="cancelOrdinaryNote" class="simple-return" type="button">CANCEL</button></div></section>`;
    const text = document.getElementById("ordinaryNoteText");
    text.value = existingText;
    const preserveDraft = () => {
      const purposeChoices = Array.from(content.querySelectorAll('.section-purpose-choices input:checked')).map(input => input.value);
      try {
        localStorage.setItem(noteDraftKey, JSON.stringify({ text: text.value, section_id: settings.section_id, feature_id: settings.feature_id, observation_id: settings.observation_id, purpose: settings.purpose, purpose_choices: purposeChoices, updated_at: new Date().toISOString() }));
        document.getElementById("ordinaryNoteState").textContent = "NOT YET SAVED - draft preserved on this phone";
        return true;
      } catch (error) {
        document.getElementById("ordinaryNoteState").textContent = `NOT SAVED - this text is still on screen. ${error.message}`;
        document.getElementById("ordinaryNoteState").dataset.kind = "error";
        return false;
      }
    };
    text.addEventListener("input", preserveDraft);
    content.querySelectorAll('.section-purpose-choices input').forEach(input => input.addEventListener("change", preserveDraft));
    document.getElementById("saveOrdinaryNote").addEventListener("click", () => {
      const saveButton = document.getElementById("saveOrdinaryNote");
      const state = document.getElementById("ordinaryNoteState");
      const noteText = text.value.trim();
      const purposeChoices = Array.from(content.querySelectorAll('.section-purpose-choices input:checked')).map(input => input.value);
      preserveDraft();
      if (!noteText && !purposeChoices.length) {
        state.textContent = "NOT SAVED - enter a note or choose a purpose.";
      ...
      try {
        const position = latestSavedPosition();
        const noteNumber = Number(data.simple_counters.NOTE || 0) + 1;
        data.simple_counters.NOTE = noteNumber;
        const noteId = `NOTE-${String(noteNumber).padStart(3, "0")}`;
        const recordedAt = new Date().toISOString();
        const marker = markerFromPosition("note", noteText, null, recordedAt, position, {
          attributes: {
            note_id: noteId, inspection_id: data.inspection_id, active_parcel: data.property_id,
            section_id: settings.section_id, feature_id: settings.feature_id, linked_observation_id: settings.observation_id,
            purpose_choices: purposeChoices, save_status: "DURABLY_SAVED", correction_history: [],
            gps_basis: lastPosition ? "CURRENT_OR_RECENT_FIX" : (position ? "LAST_SAVED_FIX" : "LOCATION_UNAVAILABLE")
          }
        });
        marker.note_id = noteId;
        marker.record_class = "field_note";
        marker.save_status = "DURABLY_SAVED";
        marker.correction_history = [];
        data.markers.push(marker);
        if (settings.section_id) {
          const section = sectionMappingTools.sectionById(data, settings.section_id);
          const sectionNote = { note_id: noteId, section_id: settings.section_id, exact_text: noteText, purpose_choices: purposeChoices, recorded_at: recordedAt, latitude: marker.lat, longitude: marker.lon, gps_accuracy_m: marker.gps_accuracy_m, inspection_id: data.inspection_id, active_parcel: data.property_id, linked_feature_id: settings.feature_id, linked_observation_id: settings.observation_id, save_status: "DURABLY_SAVED", correction_history: [], append_only: true };
          data.section_notes.push(sectionNote);
          if (section) {
            section.note_ids = Array.isArray(section.note_ids) ? section.note_ids : [];
            section.note_ids.push(noteId);
            section.purpose_choices = Array.from(new Set([].concat(section.purpose_choices || [], purposeChoices)));
            section.updated_at = recordedAt;
          }
        }
        saveState();
        const verified = JSON.parse(localStorage.getItem(stateKey) || "null");
        if (!verified || !Array.isArray(verified.markers) || !verified.markers.some(item => item.note_id === noteId)) throw new Error("The saved note could not be read back.");
        try { localStorage.removeItem(noteDraftKey); } catch (error) { /* Verified note remains saved even if the draft copy cannot be removed. */ }
        redraw();
        if (typeof settings.after_save === "function") {
          simpleSetStatus(`NOTE SAVED - ${noteId}`, "saved");
          settings.after_save(marker);
          return;
        }
        content.innerHTML = `<section class="simple-capture note-editor"><h2>NOTE SAVED</h2><div class="note-saved-confirmation"><strong>${noteId}</strong><span>${new Date(recordedAt).toLocaleTimeString()}</span><span>${settings.section_id || "General field note"}</span></div><p>${noteText || purposeChoices.join("; ")}</p><button id="noteSavedContinue" class="simple-return" type="button">CONTINUE</button></section>`;
        document.getElementById("noteSavedContinue").addEventListener("click", () => noteReturnScreen(settings));
        simpleSetStatus(`NOTE SAVED - ${noteId}`, "saved");
      } catch (error) {
        saveButton.disabled = false;
        saveButton.textContent = "RETRY SAVE";
        state.textContent = `NOT SAVED - ${error.message} Your text is still here.`;
        state.dataset.kind = "error";
        simpleSetStatus(`NOTE NOT SAVED - ${error.message} Tap RETRY SAVE.`, "warning");
      }
    });
    const saveWithout = document.getElementById("saveWithoutSectionNote");
    if (saveWithout) saveWithout.addEventListener("click", () => {
      if (!confirm("Save this section without explaining why?")) return;
      if (typeof settings.without_note === "function") settings.without_note();
    });
    document.getElementById("cancelOrdinaryNote").addEventListener("click", () => {
      preserveDraft();
      noteReturnScreen(settings);
      simpleSetStatus("NOTE NOT SAVED - draft remains on this phone.", "warning");
    });
  ...
  function simpleFeaturePrefix(type) {
    const prefixes = { water: "WATER", tree: "TREE", ditch: "DITCH", culvert: "CULVERT", brush: "BRUSH", blocked: "BLOCKED", entrance: "ROAD", open: "OPEN", highlow: "GROUND", other: "OTHER", photo: "PHOTO", note: "NOTE", site_sound: "SOUND" };
  ...
  function simpleMarkerType(type) {
    return ({ water: "wet", ditch: "ditch", brush: "thick", entrance: "entrance", open: "open", highlow: "high", photo: "photo", note: "note" })[type] || type;
  }
  ...
  function recordUiAction(eventType, details) {
    data.ui_action_log = Array.isArray(data.ui_action_log) ? data.ui_action_log : [];
    const activeSection = sectionMappingTools && sectionMappingTools.activeSection(data);
    data.ui_action_log.push(Object.assign({
      event_type: eventType,
      recorded_at: new Date().toISOString(),
      current_screen: document.querySelector("#simpleContent h2")?.textContent || "FIELD_BUTTONS",
      inspection_state: data.started ? (data.stopped ? "STOPPED" : "ACTIVE") : "NOT_STARTED",
      section_state: activeSection ? `${activeSection.section_id}:${activeSection.capture_paused ? "PAUSED" : activeSection.completion_status}` : "NONE",
      app_version: APP_VERSION,
      build: BUILD_ID
    }, details || {}));
    if (data.ui_action_log.length > 2000) data.ui_action_log = data.ui_action_log.slice(-2000);
    try { saveState(); } catch (error) { /* Diagnostics must never interrupt field capture. */ }
  }

  async function runFieldButtonCheck() {
    const button = document.getElementById("simpleButtonCheck");
    const result = document.getElementById("simpleButtonCheckResult");
    button.disabled = true;
    button.textContent = "TESTING FIELD BUTTONS...";
    result.textContent = "Checking controls, offline files, local storage, and photo storage.";
    const failures = [];
    try {
      const requiredFunctions = [renderSimpleHome, renderNoteEditor, simpleTakePhoto, renderSectionStart, renderSectionActive, renderSectionReview, renderSimpleFinish, bindSimpleLocator];
      if (requiredFunctions.some(item => typeof item !== "function")) failures.push("a required screen action did not load");
      const testKey = "propertyInspectorFixedButtonSelfCheck";
      localStorage.setItem(testKey, APP_VERSION);
      if (localStorage.getItem(testKey) !== APP_VERSION) failures.push("local saving did not read back");
      localStorage.removeItem(testKey);
      await openPhotoDb();
      await revalidatePhotoDb();
      if (!document.getElementById("photoInput")) failures.push("camera control is missing");
      if (!document.getElementById("simpleTopFinish")) failures.push("finish control is missing");
      if (document.querySelector("dialog[open]")) failures.push("a hidden dialog is blocking the screen");
    } catch (error) {
      failures.push(error.message);
    }
    if (failures.length) {
      fieldButtonCheckPassed = false;
      sessionStorage.removeItem("propertyInspectorFixedButtonCheck");
      result.textContent = `DO NOT ENTER FIELD — ${failures.join("; ")}`;
      result.dataset.kind = "error";
      button.disabled = false;
      button.textContent = "RETRY FIELD BUTTON TEST";
      recordUiAction("UI_ACTION_FAILED", { action_name: "START_OF_DAY_BUTTON_CHECK", button_id: "simpleButtonCheck", error_message: failures.join("; ") });
      return;
    }
    fieldButtonCheckPassed = true;
    sessionStorage.setItem("propertyInspectorFixedButtonCheck", APP_VERSION);
    recordUiAction("UI_ACTION_SUCCEEDED", { action_name: "START_OF_DAY_BUTTON_CHECK", button_id: "simpleButtonCheck" });
    result.textContent = "ALL REQUIRED BUTTONS RESPONDED";
    result.dataset.kind = "saved";
    simpleSetStatus("ALL REQUIRED BUTTONS RESPONDED - safe to start the test inspection.", "saved");
    setTimeout(renderSimpleHome, 400);
  }
  ...
  function renderSimpleHeader() {
    ...
    if (counts) counts.textContent = `${data.photos.length} photos | ${data.markers.length} records`;
  }
  ...
  function renderSiteSoundCapture(session) {
    ...
    const content = document.getElementById("simpleContent");
    content.innerHTML = `<section class="simple-capture"><h2>${record.sound_id} - SITE SOUND / EXPERIENCE</h2><p class="frontage-instruction">What can you hear or experience here right now?</p><p class="simple-next-line">Optional. Choose what you hear or add an ordinary text note. The app does not record audio.</p><div class="culvert-factor-list sound-choice-list">${choices.map(choice => `<label><input type="checkbox" value="${choice}" ${(record.selected_experiences || []).includes(choice) ? "checked" : ""}> ${choice}</label>`).join("")}</div><p id="soundConflict" class="frontage-warning" hidden>“Mostly quiet” conflicts with another selected sound. Both will remain saved for review.</p><div class="simple-capture-actions"><button id="soundNote" type="button">ADD NOTE</button><button id="soundSave" class="simple-return" type="button">SAVE AND CONTINUE</button><button id="soundSkip" type="button">SKIP AND CONTINUE</button></div></section>`;
    const updateChoices = () => {
    ...
    document.getElementById("soundNote").addEventListener("click", () => renderNoteEditor({ feature_id: record.sound_id, observation_id: session.observation_id, session_id: session.simple_session_id, purpose: "SITE_EXPERIENCE_NOTE" }));
  ...
  function activateSectionSession(section, returnScreen, positionOverride) {
    if (currentSimpleSession()) simpleFinalizeActive("BASIC_RECORD_SAVED_DETAILS_INCOMPLETE");
    const now = new Date().toISOString();
    const sessionPosition = positionOverride || lastPosition || (data.points.length ? data.points[data.points.length - 1] : null) || section.start || null;
    let observationId = section.observation_id || null;
    if (!observationId) {
      const marker = markerFromPosition("other", "Mapped land section", null, now, sessionPosition, {
        informationClass: "OBSERVED_ON_SITE",
    ...
    const session = {
      ...
      lat: sessionPosition && Number.isFinite(Number(sessionPosition.lat)) ? Number(sessionPosition.lat) : null,
      lon: sessionPosition && Number.isFinite(Number(sessionPosition.lon)) ? Number(sessionPosition.lon) : null,
      gps_accuracy_m: sessionPosition && Number.isFinite(Number(sessionPosition.accuracy_m)) ? Number(sessionPosition.accuracy_m) : null,
      gps_position_at: sessionPosition ? sessionPosition.time || null : null,
      gps_basis: positionOverride && positionOverride === lastPosition ? "CURRENT_FIX" : (sessionPosition ? "LAST_SAVED_FIX" : "NO_FIX_AVAILABLE")
    };
    data.simple_sessions.push(session);
    data.active_simple_session_id = session.simple_session_id;
    simpleActiveSessionId = session.simple_session_id;
  ...
  function renderSectionStart(message, options) {
    ...
    const sectionStartButton = document.getElementById("sectionStartWalking");
    sectionStartButton.textContent = "START MAPPING THIS SECTION";
    sectionStartButton.insertAdjacentHTML("afterend", `<div id="sectionStartFeedback" class="section-start-feedback" data-kind="waiting"><strong>NOT STARTED YET</strong><span>Tap the green button when you are ready to walk or mark this section.</span></div>`);
    ...
    sectionStartButton.addEventListener("click", () => {
      const feedback = document.getElementById("sectionStartFeedback");
      const startPosition = lastPosition || (data.points.length ? data.points[data.points.length - 1] : null);
      sectionStartButton.disabled = true;
      sectionStartButton.textContent = "STARTING...";
      feedback.dataset.kind = "starting";
      feedback.innerHTML = "<strong>STARTING SECTION</strong><span>Saving the section number, description, time, and GPS.</span>";
      if (!startPosition) {
        feedback.dataset.kind = "warning";
        feedback.innerHTML = "<strong>SECTION NOT STARTED</strong><span>GPS has not saved a location yet. Keep this screen open; allow Precise Location, then tap the green button again.</span>";
        sectionStartButton.disabled = false;
        sectionStartButton.textContent = "TRY STARTING THIS SECTION AGAIN";
        simpleSetStatus("SECTION NOT STARTED - GPS has not saved a location yet. Nothing was lost.", "warning");
        return;
      }
      ...
      try {
        ...
        section.gps_start_basis = startPosition === lastPosition ? "CURRENT_FIX" : "LAST_SAVED_FIX";
        activateSectionSession(section, "FIELD_BUTTONS", startPosition);
        ...
        feedback.dataset.kind = "warning";
        feedback.innerHTML = `<strong>SECTION NOT STARTED</strong><span>${error.message}</span>`;
        sectionStartButton.disabled = false;
        sectionStartButton.textContent = "TRY STARTING THIS SECTION AGAIN";
        simpleSetStatus(`SECTION NOT STARTED - ${error.message}`, "warning");
      }
    });
  ...
  function renderSectionActive(section) {
    ...
    content.innerHTML = `${simpleLocatorMarkup()}<section class="simple-capture"><h2>ACTIVE SECTION: ${section.section_id}</h2><p class="frontage-instruction">${section.capture_paused ? "SECTION EDGE PAUSED" : section.method_label}</p><p><strong>${sectionMappingTools.effectiveDescriptions(section).join(" + ")}</strong></p><div class="simple-calculation">${sectionDistanceText(section)}<br>${section.raw_walked_edge_points.length} GPS edge points | ${section.marked_corners.length} marked corners</div><div class="simple-capture-actions">${cornerButton}<button id="sectionPurposeNote" class="section-purpose-note" type="button">WHY I MAPPED THIS SECTION</button><button id="sectionAddPhoto" type="button">ADD PHOTO</button><button id="sectionLandChanged" type="button">LAND CHANGED HERE</button><button id="sectionPause" type="button">${section.capture_paused ? "RESUME" : "PAUSE"}</button><button id="sectionFinish" class="frontage-end" type="button">FINISH SECTION</button><button id="sectionCannotWalk" type="button">CANNOT WALK THE REST</button><button id="sectionReturn" class="simple-return" type="button">SAVE WHAT I HAVE & RETURN TO FIELD BUTTONS</button></div></section>`;
    const heading = content.querySelector(".simple-capture h2");
    heading.insertAdjacentHTML("beforebegin", `<div class="section-active-confirmation" data-paused="${section.capture_paused}"><strong>${section.capture_paused ? "SECTION PAUSED - SAVED SO FAR" : "SECTION STARTED AND SAVING"}</strong><span>${section.section_id} is active. ${section.capture_paused ? "Tap Resume to continue adding GPS points." : "Walk the edge or mark corners. GPS points save automatically."}</span></div>`);
    const calculation = content.querySelector(".simple-calculation");
    calculation.insertAdjacentHTML("afterend", `<p class="section-note-summary">${(section.note_ids || []).length} saved section note${(section.note_ids || []).length === 1 ? "" : "s"}</p>`);
    document.getElementById("sectionAddPhoto").textContent = "PHOTO IN THIS SECTION";
    document.getElementById("sectionAddPhoto").insertAdjacentHTML("afterend", `<button id="sectionAddWater" type="button">WATER IN THIS SECTION</button><button id="sectionAddTree" type="button">TREE IN THIS SECTION</button>`);
    document.getElementById("sectionFinish").textContent = "SAVE THIS SECTION";
    ...
    document.getElementById("sectionAddWater").addEventListener("click", () => openSimpleCapture("water", "SECTION_ACTIVE", { section_id: section.section_id }));
    document.getElementById("sectionAddTree").addEventListener("click", () => openSimpleCapture("tree", "SECTION_ACTIVE", { section_id: section.section_id }));
    document.getElementById("sectionPurposeNote").addEventListener("click", () => renderNoteEditor({ section_id: section.section_id, feature_id: section.section_id, observation_id: section.observation_id, purpose: "SECTION_PURPOSE" }));
  ...
  function attemptFinishSection(section, cannotWalk, note) {
    renderNoteEditor({
      title: "WHY DID YOU MAP THIS SECTION?",
      section_id: section.section_id,
      feature_id: section.section_id,
      observation_id: section.observation_id,
      purpose: "SECTION_FINAL_PURPOSE",
      save_label: "SAVE NOTE AND SECTION",
      allow_without_note: true,
      after_save: () => completeSectionAfterPurpose(section, cannotWalk, note),
      without_note: () => completeSectionAfterPurpose(section, cannotWalk, note)
    });
  }

  function completeSectionAfterPurpose(section, cannotWalk, note) {
  ...
  function renderSectionReview(section) {
    ...
    section.voice_note_ids = data.voice_notes.filter(note => String(note.section_id || "") === String(section.section_id)).map(note => note.id);
    ...
    const content = document.getElementById("simpleContent");
    content.innerHTML = `${simpleLocatorMarkup()}<section class="simple-capture"><h2>${section.section_id} SAVED</h2><div class="simple-calculation">Approximate area: ${area}<br>Approximate perimeter: ${section.approximate_perimeter_m == null ? "not calculated" : `${Math.round(section.approximate_perimeter_m * 3.28084)} feet`}<br>${section.calculation_label}</div><p><strong>${sectionMappingTools.effectiveDescriptions(section).join(" + ")}</strong></p>${inferred}<p>${photos.length} photo | ${(section.note_ids || []).length} section note</p><div class="simple-capture-actions"><button id="sectionNext" class="frontage-end" type="button">MAP THE NEXT SECTION</button><button id="sectionCorrect" type="button">ADD OR CORRECT DESCRIPTION</button><button id="sectionReviewNote" class="section-purpose-note" type="button">ADD WHY I MAPPED THIS SECTION</button><button id="sectionReviewPhoto" type="button">ADD PHOTO</button><button id="sectionReviewReturn" class="simple-return" type="button">RETURN TO FIELD BUTTONS</button></div></section>`;
    ...
    document.getElementById("sectionReviewNote").addEventListener("click", () => renderNoteEditor({ section_id: section.section_id, feature_id: section.section_id, observation_id: section.observation_id, purpose: "SECTION_PURPOSE" }));
  ...
  function renderOpenRevealActive(lane) {
    ...
    document.getElementById("openRevealNote").addEventListener("click", () => { const session = currentSimpleSession(); renderNoteEditor({ feature_id: lane.lane_id, observation_id: session && session.observation_id, session_id: session && session.simple_session_id, purpose: "OPEN_REVEAL_NOTE" }); });
  ...
  function renderFrontageStepOne() {
    ...
    document.getElementById("frontageStepNote").addEventListener("click", () => renderNoteEditor({ purpose: "FRONTAGE_NOTE", return_screen: "STEP_1" }));
  ...
  function renderFrontageEndSaved() {
    ...
    document.getElementById("frontageEndNote").addEventListener("click", () => renderNoteEditor({ feature_id: session.feature_id, observation_id: session.observation_id, session_id: session.simple_session_id, purpose: "FRONTAGE_END_NOTE" }));
  ...
  function renderFrontageWalk() {
    ...
    document.getElementById("frontageWalkNote").addEventListener("click", () => renderNoteEditor({ purpose: "FRONTAGE_WALK_NOTE", return_screen: "FRONTAGE_WALK" }));
  ...
  function culvertVoiceButton(session) {
    return `<button id="culvertOptionalNote" type="button">ADD NOTE</button><button id="culvertReturnField" class="simple-return" type="button">SAVE WHAT I HAVE & RETURN TO FIELD BUTTONS</button>`;
  }

  function bindCulvertCommon(session) {
    const note = document.getElementById("culvertOptionalNote");
    if (note) note.addEventListener("click", () => renderNoteEditor({ feature_id: session.feature_id, observation_id: session.observation_id, session_id: session.simple_session_id, purpose: "CULVERT_NOTE" }));
  ...
  function renderFrontageSupportCapture() {
    ...
    document.getElementById("frontageSupportNote").addEventListener("click", () => form.elements.namedItem("note").focus());
  ...
  function renderPredictionCheck() {
    ...
    content.innerHTML = `<section class="simple-capture"><h2>WHAT IS THE LIGHTER EASTERN AREA ACTUALLY LIKE?</h2><label>Prediction being checked<select id="predictionTrace"><option value="">THINNER TREE CANOPY / LIKELY LOW BRUSH OR MARSH — NO TRACE SELECTED</option>${traces.map(trace => `<option value="${trace.trace_id}">${trace.trace_id} — ${trace.trace_type}</option>`).join("")}</select></label><p class="simple-next-line">Tap every answer that is true. Each tap saves immediately. Nothing is required.</p><div id="predictionSavedCount" class="simple-calculation">0 choices saved</div><div class="simple-capture-actions">${choices.map(choice => `<button type="button" data-prediction-choice="${choice}">${choice}</button>`).join("")}<button id="predictionPhoto" type="button">PHOTO</button><button id="predictionNote" type="button">ADD NOTE</button><button id="predictionDone" class="simple-return" type="button">SAVE WHAT I HAVE & RETURN TO FIELD BUTTONS</button><button id="predictionBack" class="simple-return" type="button">RETURN TO AERIAL REVIEW</button></div></section>`;
    ...
    document.getElementById("predictionNote").addEventListener("click", () => { if (!activeCheck) { simpleSetStatus("Tap what is actually here first, or return without recording.", "warning"); return; } renderNoteEditor({ feature_id: activeCheck.field_prediction_check_id, purpose: "AERIAL_PREDICTION_NOTE" }); });
  ...
  function renderPredictionSaved(check) {
    ...
    document.getElementById("predictionNote").addEventListener("click", () => { const session = makeSession(); renderNoteEditor({ feature_id: session.feature_id, observation_id: session.observation_id, session_id: session.simple_session_id, purpose: "AERIAL_PREDICTION_NOTE" }); });
  ...
  function renderSimpleHome() {
    ...
    if (!data.started) {
      const checkReady = offlineReady && fieldButtonCheckPassed;
      content.innerHTML = `<section class="simple-start"><h2>ONE THING TO DO NEXT</h2><p>First tap TEST FIELD BUTTONS. Start becomes available only after the field controls and storage respond.</p><button id="simpleButtonCheck" type="button">TEST FIELD BUTTONS</button><div id="simpleButtonCheckResult" class="simple-calculation">${fieldButtonCheckPassed ? "ALL REQUIRED BUTTONS RESPONDED" : "BUTTON CHECK NOT RUN"}</div><button id="simpleStart" type="button" ${checkReady ? "" : "disabled"}>${!offlineReady ? "PREPARING OFFLINE USE..." : (fieldButtonCheckPassed ? "START TEST INSPECTION" : "RUN BUTTON CHECK FIRST")}</button><p class="simple-help">This fixed test uses separate storage. It cannot clear or change the saved production inspection.</p></section>`;
      document.getElementById("simpleButtonCheck").addEventListener("click", runFieldButtonCheck);
    ...
    const recoveredSession = currentSimpleSession();
    if (recoveredSession && recoveredSession.feature_type === "map_section") {
      const recoveredSection = sectionRecordForSession(recoveredSession);
      if (recoveredSection && recoveredSection.completion_status === "ACTIVE") {
        renderSectionActive(recoveredSection);
        simpleSetStatus(`${recoveredSection.section_id} IS STILL ACTIVE - continuing the same saved section`, "saved");
        return;
      }
    }
    if (["NOT_STARTED", null, undefined].includes(arrival.screen)) {
      // The frontage walk is optional. A newly started inspection must always
      // land on the simple field-button grid so an old guided workflow cannot
      // block Water, Tree, Photo, Note, or section capture.
      arrival.status = "AVAILABLE_OPTIONALLY";
      arrival.screen = "FIELD_BUTTONS";
    ...
    content.innerHTML = `${simpleLocatorMarkup()}<section class="simple-next"><strong>WHAT DO I DO NOW?</strong><span>Tap what you see. Take a photo or add a note if useful. Nothing else is required.</span></section>${resumeSection}<div class="simple-grid">
      ...
      <button id="simpleAddNote" type="button" class="simple-feature note">ADD NOTE</button>
      <button id="simpleFinish" type="button" class="simple-feature finish">FINISH</button></div>
      <details class="simple-advanced"><summary>OPTIONAL TOOLS</summary><p>The ordinary field buttons above are all you need for normal capture.</p><button id="simpleResumeFrontage" type="button">${frontageResumeLabel}</button><button id="simpleAugust4Review" type="button">AUGUST 4 REFERENCE</button><button id="simpleOpenReveal" type="button">OPEN AND REVEAL</button><button id="simpleOpenAdvanced" type="button">OPEN ORIGINAL DETAILED TOOLS</button></details>`;
    ...
    document.getElementById("simpleAddNote").addEventListener("click", () => renderNoteEditor());
  ...
  function renderSimpleCapture() {
    ...
    const photos = simpleSessionPhotos(session);
    const currentPhoto = photos.length ? photos[photos.length - 1] : null;
    content.innerHTML = `<section class="simple-capture"><h2>${session.feature_id} - ${simpleFeatureNames[session.feature_type]}</h2><p class="simple-next-line">Optional details. You can save and leave now.</p><div id="simplePhotoPreview" class="simple-photo-preview"></div><form id="simpleCaptureForm">${simpleFieldsFor(session.feature_type)}<label>Note — type or use the iPhone keyboard microphone<textarea name="note" placeholder="optional"></textarea></label></form><div id="simpleDbh" class="simple-calculation"></div><div class="simple-item-count">${photos.length} photo${photos.length === 1 ? "" : "s"}</div><div class="simple-capture-actions"><button id="simpleSaveReturn" class="simple-return" type="button">SAVE WHAT I HAVE & RETURN TO FIELD BUTTONS</button><button id="simpleTakePhoto" type="button">${photos.length ? `ADD ANOTHER PHOTO - NEXT ${simpleNextPhotoIdentifier()}` : `TAKE ${simpleNextPhotoIdentifier()}`}</button><button id="simpleFocusNote" type="button">ADD OR EDIT NOTE</button><button id="simpleQuickMeasurement" type="button">ADD QUICK MEASUREMENT</button><details><summary>ADVANCED DETAILS</summary><p>Advanced tools are optional and never required to save.</p><button id="simpleAdvancedFromCapture" type="button">OPEN ADVANCED TOOLS</button></details></div></section>`;
    const form = document.getElementById("simpleCaptureForm");
  ...
  async function openSimpleCapture(type, returnScreen, context) {
    const captureContext = context || {};
    if (type === "note") { renderNoteEditor({ section_id: captureContext.section_id || null }); return; }
    simpleSetStatus(`${simpleFeatureNames[type] || type} TAP RECEIVED - saving now.`, "saved");
    const freshPosition = await ensureFieldGpsReady();
    const capturePosition = freshPosition || latestSavedPosition();
    if (currentSimpleSession()) simpleFinalizeActive("BASIC_RECORD_SAVED_DETAILS_INCOMPLETE");
    simpleCloseDialogs();
    ...
    const session = {
      ...
      section_id: captureContext.section_id || null,
      lat: capturePosition && Number.isFinite(Number(capturePosition.lat)) ? Number(capturePosition.lat) : null,
      lon: capturePosition && Number.isFinite(Number(capturePosition.lon)) ? Number(capturePosition.lon) : null,
      gps_accuracy_m: capturePosition && Number.isFinite(Number(capturePosition.accuracy_m)) ? Number(capturePosition.accuracy_m) : null,
      gps_position_at: capturePosition ? capturePosition.time || null : null,
      gps_basis: freshPosition ? "CURRENT_FIX" : (capturePosition ? "LAST_SAVED_FIX" : "LOCATION_UNAVAILABLE"),
      compass_heading_deg: latestOrientation ? latestOrientation.compass_heading_deg : (capturePosition ? capturePosition.heading_deg : null),
    ...
    const marker = markerFromPosition(simpleMarkerType(type), "", null, now, capturePosition, { attributes: { simple_session_id: session.simple_session_id, feature_id: featureId, simple_feature_type: type, section_id: session.section_id, completion_status: "ACTIVE", gps_basis: session.gps_basis } });
  ...
  function simpleTakePhoto(photoRole) {
    ...
    takePhoto({
      ...
      evidenceClassification: "Observed",
      observationAttributes: Object.assign({}, session.details, { simple_session_id: session.simple_session_id, feature_id: session.feature_id, section_id: session.section_id || null, photo_role: photoRole || "Context" }),
      simple_capture: true,
      simple_session_id: session.simple_session_id,
      feature_id: session.feature_id,
      section_id: session.section_id || null,
  ...
  function simpleSaveAndReturn() {
    try {
      ...
      const returnSectionId = active && active.section_id;
      ...
      if (returnScreen === "SECTION_ACTIVE" && returnSectionId) {
        const section = sectionMappingTools && sectionMappingTools.sectionById(data, returnSectionId);
        if (section && section.completion_status === "ACTIVE") {
          activateSectionSession(section, "FIELD_BUTTONS");
          renderSectionActive(section);
        } else {
          setFrontageScreen("FIELD_BUTTONS");
          renderSimpleHome();
        }
      }
      else if (returnScreen && returnScreen !== "FIELD_BUTTONS") { setFrontageScreen(returnScreen); renderFrontageWorkflow(); }
  ...
  async function initialize() {
    const buildLine = document.getElementById("simpleBuildLine");
    if (buildLine) buildLine.textContent = `APP VERSION: ${APP_VERSION} | BUILD: ${BUILD_ID}`;
    const shellVersion = document.querySelector('meta[name="property-inspector-build"]')?.content || "";
    if (shellVersion !== APP_VERSION) {
      simpleSetStatus("APP FILES DO NOT MATCH — DO NOT BEGIN INSPECTION", "warning");
      document.getElementById("simpleContent").innerHTML = `<section class="simple-start"><h2>APP FILES DO NOT MATCH — DO NOT BEGIN INSPECTION</h2><p>Tap RELOAD LATEST APP above. Existing inspection evidence has not been changed.</p></section>`;
      return;
    }
    ...
    loadState();
    try {
      const response = await fetch("../field-simple-test/assets/august-4-route-context.json");
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
    ...
    simpleShell.addEventListener("click", event => {
      ...
      if (button.id !== "simpleButtonCheck") simpleSetStatus(`${details.action_name} — TAP RECEIVED`, "normal");
      setTimeout(() => {
    ...
    document.getElementById("reloadLatestApp").addEventListener("click", async () => {
      simpleSetStatus("CHECKING FOR THE LATEST VERIFIED APP — saved evidence is unchanged.", "normal");
      try {
  ...
  window.addEventListener("pageshow", () => {
    ...
    simpleSetStatus(data.started ? "APP RESTORED — saved work is still here. GPS is resuming." : "LATEST VERIFIED BUILD LOADED", "saved");
  });
  ...
  window.addEventListener("unhandledrejection", event => {
    ...
    const report = document.getElementById("reportButtonFailure");
    if (report) report.hidden = false;
    simpleSetStatus(`ACTION FAILED — ${message}. Your saved evidence remains on this phone.`, "warning");
  });
