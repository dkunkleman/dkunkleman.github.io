(function (root, factory) {
  ...
  function createPhotoPointsGeoJSON(photos) {
    return {
      type: "FeatureCollection",
      name: "Inspection photograph points",
      features: (photos || []).filter(photo => photo.location && Number.isFinite(Number(photo.location.longitude)) && Number.isFinite(Number(photo.location.latitude))).map(photo => ({
        type: "Feature",
        properties: {
          record_type: "photograph",
          photo_id: photo.photo_id,
          photo_number: photo.photo_number,
          simple_photo_id: photo.simple_photo_id || null,
          active_feature_id: photo.feature_id || null,
          sequence_within_feature: photo.simple_feature_sequence || null,
          recorded_at: photo.recorded_at,
          gps_accuracy_m: photo.location.accuracy_m,
          compass_heading_deg: photo.compass_heading_deg,
          analysis_path: photo.analysis && photo.analysis.path || null,
          original_path: photo.original && photo.original.path || null
        },
        geometry: { type: "Point", coordinates: [Number(photo.location.longitude), Number(photo.location.latitude)] }
      }))
    };
  }

  function createFeaturePointsGeoJSON(markers) {
    return {
      type: "FeatureCollection",
      name: "Inspection feature points",
      features: (markers || []).filter(item => Number.isFinite(Number(item.lon)) && Number.isFinite(Number(item.lat)) && item.type !== "photo" && item.type !== "voice_note").map(item => ({
        type: "Feature",
        properties: {
          record_type: "field_feature",
          event_id: item.id,
          feature_id: item.attributes && item.attributes.feature_id || null,
          simple_session_id: item.attributes && item.attributes.simple_session_id || null,
          feature_type: item.attributes && item.attributes.simple_feature_type || item.type,
          label: item.button_label || item.type,
          recorded_at: item.time,
          gps_accuracy_m: item.gps_accuracy_m,
          compass_heading_deg: item.compass_heading_deg,
          note: item.note || "",
          attributes: cloneWithoutBinary(item.attributes || {})
        },
  ...
  function createRepositoryImportManifest(manifest, fileName) {
    ...
    return {
      ...
      artifact: {
        ...
        repository_filename: `HOME_TEST_3_13_${manifest.package_mode === "full_evidence_archive" ? "FULL_ARCHIVE" : "AI_ANALYSIS_REPORT_PACKAGE"}_${exportId}.zip`,
  ...
  function makeReadme(manifest) {
    ...
    return [
      ...
      "- subject-parcel.geojson: the subject parcel outline copied into this package; approximate public parcel context, not a survey.",
      "- photo-points.geojson: every photograph location with photo ID, active feature ID, sequence, GPS accuracy, time, and heading.",
      "- feature-points.geojson: each saved field feature with its permanent feature ID and direct attributes.",
      "- property-field-map.html and printable-property-field-map.html: self-contained route, parcel, feature, and photo map/report views suitable for screen or print.",
...
${questions.map(question => `## ${question.category}\n\n${question.question}\n\n- Audience: ${question.stakeholder}\n- Decision: ${question.decision_id || "Cross-cutting"}\n- Evidence present: ${question.evidence_present ? "Yes" : "No or not specifically recorded"}\n- Supporting observations: ${question.supporting_observation_ids.join(", ") || "None specifically linked"}\n- Output rule: Answer when supported. Otherwise state the uncertainty and the lowest-cost credible way to resolve it.\n`).join("\n")}`;
  ...
  async function createInspectionPackage(options) {
    ...
    const fileName = `HOME_TEST_3_13_${propertyFolder}_Inspection_${includeOriginals ? "FULL_ARCHIVE" : "AI_ANALYSIS_REPORT_PACKAGE"}_${settings.packageKind === "backup" ? "Backup_" : ""}${packageTimestamp(exportedAt)}_${exportId}.zip`;
    ...
    const mappedSectionsGeoJson = {
      ...
      features: (mappedSections.sections || []).flatMap(section => {
        const features = [];
        const sectionNotes = (sourceInspection.section_notes || []).filter(note => note.section_id === section.section_id);
        const noteProperties = { note_ids: sectionNotes.map(note => note.note_id), section_purposes: Array.from(new Set(sectionNotes.flatMap(note => note.purpose_choices || []))), section_note_text: sectionNotes.map(note => note.exact_text).filter(Boolean) };
        if (section.outlined_section) features.push({ type: "Feature", geometry: section.outlined_section, properties: Object.assign({ section_id: section.section_id, feature_role: "APPROXIMATE_SECTION_OUTLINE", information_class: "CALCULATED", label: section.calculation_label, descriptions: section.effective_description_selections }, noteProperties) });
        if (Array.isArray(section.walked_edge) && section.walked_edge.length > 1) features.push({ type: "Feature", geometry: { type: "LineString", coordinates: section.walked_edge.map(point => [point.longitude, point.latitude]) }, properties: Object.assign({ section_id: section.section_id, feature_role: "PHYSICALLY_WALKED_EDGE", information_class: "CAPTURED_BY_DEVICE" }, noteProperties) });
